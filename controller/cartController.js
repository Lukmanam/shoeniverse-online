        ...
        if (req.session.user_id) {
            if(cartData){
                if(cartData.products.length > 0){
                    ...
                    res.render('cart',{userData,products,Total,userId,user,cartData});
                } else {
    ...
    try {
        ...
        if (req.session.user_id) {
            ...
            if (productData.quantity > 0) {
                if (cartData) {
                    const productExist = cartData.products.find(product => product.productId.equals(productId));
                    ...
                    if (productExist) {
                        ...
                        await Cart.findOneAndUpdate(
                            { user: userId },
                            { $push: { products: { productId: productId, productPrice: productData.price } } }
                        );
                    }
                } else {
                    const cart = new Cart({
